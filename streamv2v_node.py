...
class Stream_Sampler:
    ...
    def main_stream(self,pipe,info,prompt,video,sampler_type,guidance_scale,diffusion_steps,num_inference_steps,
                    noise_strength,seed,width,height,batch_size,**kwargs):
        model_type,trigger_word=info.split(";")
        if model_type=="sdxl":
            using_sdxl=True
        else:
            using_sdxl=False
        ...
        if sampler_type=="vdieo2vdieo":
            ...
            video_info = read_video(input_video)  # input path to the file name  "THWC") -> tuple[Tensor, Tensor, dict[str, Any]]
